...
class CreateAnimalForm(forms.ModelForm):
    ...
    name = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder':'Ləqəb'}))
    age = forms.IntegerField(widget=forms.NumberInput(attrs={'class': 'form-control', 'min':0}))
    weight = forms.FloatField(widget=forms.NumberInput(attrs={'class': 'form-control','step':'0.01', 'min':0}))
    phone = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder':'xx xxx xx xx'}))
    ...
    entry_date = forms.DateTimeField(widget = forms.TextInput(attrs={'class':'form-control', }), initial=date.today())
...
class UpdateAnimalForm(forms.ModelForm):
    ...
    weight = forms.FloatField(widget=forms.NumberInput(attrs={'class': 'form-control','step':'0.01', 'min':0}))
    phone = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder':'xx xxx xx xx'}))
    note = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control', 'rows':'25', 'placeholder':'Əlavə məlumat'}))
    doctor = forms.ModelChoiceField(queryset= Doctor.objects.all(), empty_label="Həkimi seçin", widget=forms.Select(attrs={'class':'form-select'}))
    examination = forms.ModelChoiceField(queryset= MedicalExamination.objects.all(), empty_label="Müayinəni seçin", widget=forms.Select(attrs={'class':'form-select'}))
    breed = forms.ChoiceField(choices=Animal.BREEDS, widget=forms.Select(attrs={'class':'form-select'}))
    color = forms.ChoiceField(choices=Animal.COLORS, widget=forms.Select(attrs={'class':'form-select'}))
    entry_date = forms.DateTimeField(widget = forms.TextInput(attrs={'class':'form-control', }))
